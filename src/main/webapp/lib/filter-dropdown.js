...
proto.change = function (e) {
  ...
  $('.dropdown-menu li label').removeClass('radioActive');
  $('.filterDropDown').each(function( index, element ) {
      var buttonText = $(element).find('button').text(),
          containElements = $(element).find('ul li label:contains('+buttonText+')'),
          containsElementLength = containElements.length;

      for (var i = 0; i < containsElementLength; i++) {
          if ($(containElements[i]).text() === buttonText) {
              $(containElements[i]).addClass('radioActive');
          }
      }
  });
};
